...
read_and_force <- function(fn) {
  ad <- anndataR::read_h5ad(fn)
  sum(ad$X@x)
  invisible(ad)
}

# Warm up
message("Warming up anndataR ...")
...
write.csv(timings, file.path(out_dir, "timings_anndataR.csv"), row.names = FALSE)
message("Saved timings_anndataR.csv")
